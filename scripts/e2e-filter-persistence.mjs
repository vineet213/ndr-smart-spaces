...
try {
  ...
  await page.evaluate(() => {
    const pill = [...document.querySelectorAll(".filter-pill")].find((b) =>
      b.textContent.startsWith("draft"),
    );
    pill?.click();
  });
  ...
  const rowButton = await page.evaluateHandle((name) => {
    for (const row of document.querySelectorAll("#record-list-inner .record-row")) {
      if (row.textContent.includes(name)) {
        return (
          [...row.querySelectorAll("button.mini")].find(
  ...
  if (!publishBtn) {
    const diag = await page.evaluate((name) => {
      ...
      return {
        ...
        activePill: [...document.querySelectorAll(".filter-pill")].find((p) =>
        ...
        matchedRow: !!match,
        btns: match
          ? [...match.querySelectorAll("button")].map(
  ...
  record(
    "filters survive Publish (state/status/search)",
    ui.selectValue === "Tamil Nadu" &&
      ui.activePill?.startsWith("draft") &&
      ui.searchValue === SEARCH_TERM,
  ...
  record(
    "count updates to reflect new status",
    ui.count === `${expectedAfter} of ${records.length} records`,
    ui.count,
  );
  record(
    "row left the draft filter after publishing",
    ui.rows === expectedAfter,
    `${ui.rows} rows`,
  );
  ...
  record(
    ...
    nowPublished === "published",
    nowPublished,
  );

  /* Save ≠ Publish: public site must NOT know yet */
  const pubHtml = await (await fetch(PUBLIC_REGISTER)).text();
  record(
    "public site unchanged by Save-side transition (no export)",
    !pubHtml.includes(target.data.name),
  );

  /* restore */
  record(
    "restore transition published → draft",
    (await apiTransition(cookie, target.id, "draft")) === 200,
  );

  /* ── 2. edit → back keeps filters (continuing on the live filtered view) ── */
  const editBtn = await page.evaluateHandle(() =>
    [...document.querySelectorAll("#record-list-inner .record-row button")].find(
  ...
  record(
    ...
    ui.rows === records.length &&
      ui.count === `${records.length} of ${records.length} records` &&
      ui.selectValue === "all" &&
      ui.searchValue === "" &&
      !ui.clearVisible,
    ui.count,
