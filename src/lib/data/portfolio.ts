 ...
 *
 * Phase 2B — the geography (locations with their portfolio map offsets) flows
 * from the CMS-managed `locations` collection, the catalogue plates from the
 * `portfolio-assets` collection and the edition line from the
 * `publication-settings` collection (regenerated byte-identically into
...
const cmsGeoLocations: readonly GeoLocation[] = cmsLocationData
  .filter((location) => location.visible.portfolio)
  .map((location) => {
    const offset = location.portfolioOffset;
    return {
      id: location.id,
      name: location.name,
      zone: location.zone,
...
const cmsPortfolioAssetData = cmsPortfolioAssets as unknown as readonly CmsPortfolioAsset[];

const cmsPortfolioAssetsDerived: readonly PortfolioAsset[] = cmsPortfolioAssetData.map((asset) => ({
  id: asset.id,
  plate: asset.plate,
  name: asset.name,
  city: asset.city,
  ...
  ...(asset.occupier !== undefined ? { occupier: asset.occupier } : {}),
  ...(asset.completedYear !== undefined ? { completedYear: asset.completedYear } : {}),
  ...(asset.entity !== undefined ? { entity: asset.entity } : {}),
  source: asset.source,
}));

export const portfolioAssets: readonly PortfolioAsset[] = cmsPortfolioAssetsDerived;
...
/** Assets under construction (status "ongoing") — the development pipeline. */
export function underConstructionAssets(): readonly PortfolioAsset[] {
  return portfolioAssets.filter((asset) => asset.status === "ongoing");
}
...
export const portfolioRegister = {
  ...
  summary: {
    ...
    editionValue: cmsPublication.editionPeriod,
  } as const,
  columns: {
    plate: "Plate",
    asset: "Asset",
